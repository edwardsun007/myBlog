...
export class PostService {
  ...
  getPosts(postsPerPage: number, currentPage: number) {
    ...
    this._http
      .get<{ message: string; data: any; maxPosts: number }>( // Where this data from ? think
        ...
        configs.apiPrefix + "/api/posts" + queryParams // hardcoded URL point to path defined in server.js
      )
      .pipe(
        map(postData => {
      ...
      .subscribe(transformedPostData => {
        // transformedPosts will be array already
  ...
  getPost(id: string) {
    ...
    return this._http.get<{
      ...
      imagePath: string;
      creator: string;
    }>(`${configs.apiPrefix}/api/posts/${id}`);
  ...
  updatePost(id: string, title: string, content: string, image: File | string) {
    ...
    if (typeof image === "object") {
      // image is from filePicker, its File type / object
      ...
      // existing image is string, we simply pass it via json
      postData = {
        id: id,
        title: title,
        content: content,
    ...
    this._http
      .put(`${configs.apiPrefix}/api/posts/${id}`, postData)
      .subscribe(res => {
        // console.log(res);
  ...
  deletePost(id: string) {
    ...
    return this._http.delete(`${configs.apiPrefix}/api/posts/${id}`); // delete from backend
  }
}
